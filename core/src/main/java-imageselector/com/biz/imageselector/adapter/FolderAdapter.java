...
import com.biz.imageselector.bean.Folder;
...
public class FolderAdapter extends BaseAdapter {
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        ...
        if (holder != null) {
            if (i == 0) {
                holder.name.setText(R.string.imageselector_folder_all);
                holder.path.setText("/sdcard");
                holder.size.setText(String.format("%d%s",
                        getTotalImageSize(), mContext.getResources().getString(R.string.imageselector_photo_unit)));
                if (mFolders.size() > 0) {
...
//                            .setImageType(ImageType.SMALL)//图片类型
                            ...
                            .setResizeOptions(new ResizeOptions(R.dimen.imageselector_folder_cover_size, R.dimen.imageselector_folder_cover_size))//调整大小
                            .build();
    ...
    class ViewHolder {
        ...
        void bindData(Folder data) {
            ...
            if (data.images != null) {
                size.setText(String.format("%d%s", data.images.size(), mContext.getResources().getString(R.string.imageselector_photo_unit)));
            } else {
                size.setText("*" + mContext.getResources().getString(R.string.imageselector_photo_unit));
            }
            // 显示图片
            if (data.cover != null) {
//                Picasso.with(mContext)
...
//                        .into(cover);
//
                Uri uri = Uri.fromFile(new File(data.cover.path));

                ImageRequest imageRequest = ImageRequestBuilder.newBuilderWithSource(uri)
                        .setLocalThumbnailPreviewsEnabled(true)
//                        .setImageType(ImageRequest.ImageType.SMALL)//图片类型
                        .setLowestPermittedRequestLevel(ImageRequest.RequestLevel.FULL_FETCH)//请求经过缓存级别  BITMAP_MEMORY_CACHE，ENCODED_MEMORY_CACHE，DISK_CACHE，FULL_FETCH
                        .setResizeOptions(new ResizeOptions(R.dimen.imageselector_folder_cover_size, R.dimen.imageselector_folder_cover_size))//调整大小
                        .build();
                ...
                cover.setImageResource(R.drawable.imageselector_default_error);
            }
        }
    }
